...
```json
{
  "setup_prompt": "Record baseline: call list_rm_rules and note the count N. Also get_app_config on the Rule Machine PARENT app and note its hasChildren count M. Also enable MCP debug logging (update_debug_logs with action='setLevel', level='debug') so we can capture the tool's internal trace.",
