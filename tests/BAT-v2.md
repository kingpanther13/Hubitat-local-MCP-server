...
### T604 — get_hpm_drift / list_hpm_packages: hpmAppId pointing at a non-HPM app surfaces descriptive error
...
```json
{
  ...
  "test_prompt": "Call get_hpm_drift with the non-HPM appId you just found as hpmAppId. Then call list_hpm_packages with the same non-HPM appId. Confirm both tools reject it with a descriptive error naming the actual app type.",
  "teardown_prompt": "No teardown needed."
}
```
...
### T605 — list_hpm_packages: hpmAppId pointing at a non-HPM app surfaces descriptive error (standalone)
...
### T622 — addTrigger Custom Attribute '*changed*' returns partial=false (cosmetic-partial filter)
...
**Expected**: `addTrigger` returns `success=true` and `partial=false`. The trigger row bakes (mainPage shows the rendered Custom Attribute trigger, not the "Define Triggers" placeholder). `settingsSkipped` may contain a `not_in_schema` entry for the comparator/attribute field that a `*changed*` trigger does not need -- that entry is informational and must NOT flip `partial`.

**Failure modes**: `partial=true` on a fully-baked trigger (the cosmetic-partial regression -- a `not_in_schema` skip on a working trigger incorrectly flips partial). A genuine `silent_rejection` skip returning `partial=false` (over-broad filtering -- real degradation must still flip partial).
