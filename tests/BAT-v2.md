...
### T710 - Captured device states survive separate requests and overwrite cleanly

**Prompt**: "On the dedicated test hub, use two new uniquely named virtual devices: a dimmer to remember and a switch to trigger temporary legacy custom rules. First check that saving one snapshot will not evict an existing one. Set the dimmer on at level 37, then use the switch to capture it under a unique snapshot name. In separate requests, list that snapshot and read its new stored file. Change the dimmer to level 12 and off, then trigger restoration and verify on at 37. Repeat under the same snapshot name at level 68, verifying the new file, unchanged snapshot count, removal of the old file, and restoration to 68. Report observed capture and restore times. Remove the temporary rules, that exact snapshot and both devices, and verify that the original snapshots are unchanged and every test-owned payload file is gone. Allow existing snapshots to acquire files during background migration."

**Expected**: Actual child event handlers execute `capture_state` and `restore_state`; `hub_test_custom_rule` alone is only a dry-run. The file contains the exact captured device ID, switch and numeric level, with matching snapshot ID and timestamp. Separate File Manager reads establish durable file contents. Each overwrite leaves one current generation; deletion removes its owned payload. A full capture store causes a skip without evicting another snapshot. Timings include transport and event observation, with no performance threshold or speedup claim. Automated coverage: `action_types/test_capture_state_file_round_trip`.

**Optional operator-authorized reload check**: On the dedicated test hub, pause after capture, reload the MCP parent app, then trigger restoration in a fresh request and verify the saved state. Record whether reload was actually performed; the automated scenario above does not prove reload recovery. Inspect existing app-status state data to confirm the capture index carries metadata rather than device payloads when that surface is available. No runtime debug API or personal-hub operation is required by this test.
