...
```json
{
  "setup_prompt": "Hub Admin Write and Built-in App Tools are enabled. Identify a device whose attribute name the hub treats as an ENUM (e.g. a switch's 'switch' attribute, deviceId 2 on the test hub). Create RM rule 'BAT CustomEnumCond'.",
...
```json
{
  "setup_prompt": "Hub Admin Write and Built-in App Tools are enabled. Identify a device whose attribute name the hub treats as an ENUM (e.g. a switch's 'switch' attribute, deviceId 2 on the test hub). Create RM rule 'BAT WalkEnumAttr'.",
...
### T637 — addTrigger Custom Attribute comparator-guard re-fetch failure: comparator written as fallback, no abort

```json
{
  "setup_prompt": "Hub Admin Write and Built-in App Tools are enabled. Create RM rule 'BAT CustomFetchFallback'. (This scenario exercises a transient hub-fetch failure path that is not deterministically reproducible from an agent prompt against a healthy hub -- it is primarily covered by the Spock specs; the live BAT form documents the observable contract.)",
