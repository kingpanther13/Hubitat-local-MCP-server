...
Call `hub_get_tool_guide(section='performance')` for response-shape details, filter/projection semantics, and field-name reference.""",
            ...
            outputSchema: [
                ...
                properties: [
                    ...
                    source: [type: "string", description: "scope='all': the hub endpoint the inventory came from -- /device/listWithCapabilities/json, or /hub2/devicesList on platform 2.5.1.173 and later where the former is gone"],
                    capabilitiesPartial: [type: "boolean", description: "scope='all': true when the inventory came from /hub2/devicesList, which carries no capabilities; capabilities are then filled only for mcpAuthorized devices and capabilityFilter matches only those"],
                    capabilitiesNote: [type: "string", description: "scope='all': present with capabilitiesPartial, explaining which devices carry capabilities and why"],
