...
Only query devices the user has mentioned or that are relevant to their request. Do not probe random devices.""",
        ...
        [
            ...
            description: """Get a device attribute's current value, or block-poll one OR several devices until the attribute reaches an expected value.
...
Only query devices the user has mentioned or that are relevant to their request.""",
            inputSchema: [
                type: "object",
                properties: [
                    deviceId: [type: "string", description: "Device ID from hub_list_devices. Required for single-device mode; omit when using deviceIds. Provide exactly ONE of deviceId or deviceIds, not both."],
                    deviceIds: [type: "array", items: [type: "string"], description: "Poll several devices at once (multi-device convergence). Mutually exclusive with deviceId -- provide exactly one.[[FLAT_TRIM]] The same condition (attribute + comparator + expectedValue(s) + stableForMs) is applied to every device; mode controls the aggregate (any/all). Max 20 devices. The result is a compact per-device array (deviceId/device/finalValue/matched, plus per-device neverReported/nonNumericAttribute on timeout) plus convergedCount -- not full device objects.[[/FLAT_TRIM]]"],
                    mode: [type: "string", enum: ["any", "all"], description: "Multi-device only (with deviceIds): converge when ANY device matches, or when ALL match. Default all. Rejected if passed with a single deviceId.[[FLAT_TRIM]] The aggregate predicate also drives stableForMs (the whole any/all condition must hold for the window).[[/FLAT_TRIM]]", default: "all"],
                    attribute: [type: "string", description: "Attribute name (the same attribute is read on every device in multi-device mode)"],
