...
Only query devices the user has mentioned or that are relevant to their request.""",
        ...
        [
            ...
            description: """Send a command (e.g. on, off, setLevel) to a device. Use to actuate or control a device; for read-only checks use hub_get_device_attribute instead. Returns a `state` snapshot (per-attribute value + freshness timestamp) read AS OF the command.[[FLAT_TRIM]] This snapshot is an immediate read taken in the same request that fires the command, so it shows the PRE-effect value -- even for virtual/local devices -- because the hub commits the change after this request returns; the per-attribute timestamp is the freshness signal.[[/FLAT_TRIM]] To get the CONFIRMED resulting state, pass `waitFor` to block-poll until the attribute converges.[[FLAT_TRIM]] Then the `state` snapshot reflects the converged value and a `waitFor` result block reports convergence; without `waitFor`, confirm separately via hub_get_device_attribute.[[/FLAT_TRIM]]
