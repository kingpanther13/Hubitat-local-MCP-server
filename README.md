...
- **v0.9.0** - **BREAKING**: All 30 top-level tool names now prefixed with `hub_` (e.g. `list_devices` → `hub_list_devices`, `manage_logs` → `hub_manage_logs`) to prevent collisions in multi-MCP-server setups. Sub-tools within gateways unchanged. New tools: `get_performance_stats` (device/app performance stats — method call counts, % busy, state size, events) and `get_hub_jobs` (scheduled/running jobs, hub actions), both in `hub_manage_logs` gateway. Fix: `get_memory_history` now has `limit` parameter (default 100) to prevent response-too-large errors. 73 MCP tools total (30 on `tools/list`).
