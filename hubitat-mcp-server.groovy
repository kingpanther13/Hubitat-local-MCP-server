...
A peer with UI login security must have its mesh token stored here first via `hub_update_hub_mesh(peer_hub_id, peer_token)`, or the link silently no-ops (the `createLinked` GET returns 200 with an empty body even when nothing links). The link tools detect this: a confirmed non-link (the source still shows `linkedLocally:false` in `hub_get_hub_mesh`) returns a FAILURE with the peer-token hint, not a soft warning; only an unreadable mesh list warns. Get the peer's token from ITS OWN `hub_get_hub_mesh(include_token=true)`.
...
- **Link** a device a PEER hub shares onto this hub: `hub_create_device(mesh_source_hub_id, mesh_source_device_id, confirm=true)` — the pair comes from `hub_get_hub_mesh` `availableLinkedDevices[]` (`hubId` + `deviceId`). The result carries the new local `deviceId` (resolved by a `localLinkedDevices` diff). The `createLinked` GET returns 200 with an empty body even when nothing actually links, so the tool reads mesh state back and keys on the source: if the device links (it drops out of `availableLinkedDevices[]` or its row flips `linkedLocally:true`) you get `success` with the new id; if it stays present with `linkedLocally:false` (a confirmed silent no-op — most often because this hub holds no mesh token for a UI-secured peer) you get a `success:false` FAILURE with a peer-token hint, NOT a soft warning; only an unreadable mesh list is a warn-not-fail. Fix a no-op by storing the peer's token with `hub_update_hub_mesh(peer_hub_id, peer_token)` (get it from the peer's OWN `hub_get_hub_mesh(include_token=true)`), then retry.
...
- **Link** a variable a PEER hub shares onto this hub: `hub_create_variable(mesh_source_hub_id, mesh_source_name, confirm=true)` — the pair comes from `hub_get_hub_mesh` `availableLinkedHubVariables[]` (`hubId` + `name`), and is mutually exclusive with the create forms (`name`/`type`/`value` or `variables`). The `createLinkedHubVar` GET returns 200 with an empty body even when nothing links, so the tool reads mesh state back and keys on the source: if the variable links (its name appears in `localLinkedHubVariables[]`, or its `availableLinkedHubVariables[]` row flips `linkedLocally:true`) you get `success`; if it stays present with `linkedLocally:false` (a confirmed silent no-op — most often because this hub holds no mesh token for a UI-secured peer) you get a `success:false` FAILURE with a peer-token hint, NOT a soft warning; only an unreadable mesh list is a warn-not-fail. Fix a no-op by storing the peer's token with `hub_update_hub_mesh(peer_hub_id, peer_token)` (get it from the peer's OWN `hub_get_hub_mesh(include_token=true)`), then retry.
