...
- `state_change_comparator_ignored_explicit_value` -- INFORMATIONAL (does NOT flip `partial`). A no-RHS state-change comparator was requested ALONGSIDE an explicit `state`/`value` on a device-state or enum Custom Attribute trigger (a contradictory spec). The explicit value wins into the value picker and the row works as an equals-check, so the dropped change intent is reported informationally. Keyed on a synthetic non-field key (`comparator@tstate<N>` on the device-state path, the hidden `ReltDev<N>` on the Custom Attribute path) so the real value-picker field is never listed in both `settingsApplied` and `settingsSkipped`.
