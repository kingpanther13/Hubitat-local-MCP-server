 ...
 *
 * Version: 3.4.1 - Enriched list_devices summary + server-side filter (disabled, enabled, stale:N)
 *
 * Installation:
...
- **hub_list_rules** — enumerate Rule Machine rules (RM 4.x + 5.x combined, deduplicated by id). Each rule carries a live **status** — "active" | "paused" | "disabled" | "unknown" — plus **disabled** / **paused** booleans (omitted on the "unknown" path) and, only when detected, **requiredExpressionFalse: true**.
  ...
  - **precedence**: disabled > paused > active (a red-X disabled app processes no pause command and shows no "(Paused)" decoration).
  - **status "unknown"** means /hub2/appsList was momentarily unreadable — the rules are then returned unfiltered (post-delete ghosts may linger) and carry no status booleans; a result-level **statusNote** explains it. Retry.
...
Returns one entry per rule: `{appId, name, disabled, paused}`. `disabled` is the red-X flag; `paused` is detected from a "(Paused)" suffix on the rule's name (VRB has no RMUtils label to cross-check, so a rule literally named "... (Paused)" reads as paused). For the authoritative pause state, read the single rule with `hub_get_visual_rule(appId=N)` — its `rulePaused` comes straight from the builder JSON.
