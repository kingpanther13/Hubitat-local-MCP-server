...
**PRs MUST NOT commit `bundles/*.zip`.** Per-PR zip rebuilds made any two library-touching PRs conflict on the binary; that model is retired. Where each consumer's zip comes from instead:

- **Main (HPM users, the e2e restore, `hub_update_package` at `ref=main`)** — the committed zip, owned by the post-merge `rebuild-bundle.yml` workflow: after each merge to main it rebuilds from `libraries/*.groovy` and commits when drifted, so the stable raw-main URL in `packageManifest.json` always serves a zip matching main.
- **Unmerged refs (PR e2e, `hub_update_package` at a branch/SHA)** — the bot-only `bundle-artifacts` branch: `publish-bundle-artifact.yml` builds the zip on every library-touching push and stores it under `branches/<branch>/` and `shas/<full-sha>/`, always under the production basename (the hub keys the bundle entity on the zip filename — a renamed zip imports as a SECOND entity with duplicate libraries instead of updating the existing one; run 27322480301). Nobody rebases that branch, so it has zero conflict surface.
- **PR e2e** (`mcp_watchdog_deploy.sh`) builds the zip in CI, skips the install when it is byte-identical to canonical main's (no recompile wave; the run-scoped bundle-state marker still tells the dead-man restore what happened), otherwise installs the artifact URL after byte-verifying it against the CI build, and then VERIFIES every `#include`d library landed (single copy per namespace+name, on-hub length equals the PR file) — a success-but-stale bundle install can never pass the deploy step silently. A library-touching fork PR has no artifact and fails loudly with the remedy (push the branch to the base repo).
- **`hub_update_package`** resolves artifact-first (probing the artifact's `.size` marker) and falls back to the zip committed at the ref — correct whenever the ref didn't change `libraries/` — warning only when a non-main ref actually fell back.

The `sandbox-lint` workflow runs the builder in-PR purely as a build smoke (a broken `LIBS` entry fails in-PR); there is no pre-commit hook and no drift gate anymore.
...
4. Do NOT rebuild or commit `bundles/*.zip` — the post-merge `rebuild-bundle.yml` workflow owns the committed zip (see "HPM delivery" above). Run `python tools/build-bundle.py` locally only to verify the builder accepts your `LIBS` entry.
