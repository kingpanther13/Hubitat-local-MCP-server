...
A label only RE-RUNS e2e when it **flips the lane** (focused↔full), decided by the `lane-gate` job: the FIRST full-run label (focused→full) and removing the LAST one (full→focused) re-run; a redundant full-run label (a 2nd `release:*`/`e2e:full` while already full, or removing one while another remains) is a **no-op** — the prior full run's gate status still stands, so it does NOT re-deploy. A new commit (or `workflow_dispatch`) always re-runs. (To not cancel an in-flight run, `cancel-in-progress` is off for label events — a real commit/reopen/dispatch still cancels-and-supersedes.)
