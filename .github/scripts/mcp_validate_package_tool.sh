...
if [ -z "$TEXT" ]; then
  echo "::error::hub_update_package returned empty MCP content (resp head: $(echo "$RESP" | head -c 600))"
  exit 1
fi
...
echo "Tool response: success=$OK dryRun=$DRY appClassId=$APP_CLASS includes=$N_INCLUDES plannedLibraries=$N_PLANNED"

if [ "$OK" != "true" ] || [ "$DRY" != "true" ]; then
  echo "::error::hub_update_package dryRun did not succeed cleanly: $(echo "$TEXT" | jq -c '{success,dryRun,aborted,abortReason,error}')"
  exit 1
fi
