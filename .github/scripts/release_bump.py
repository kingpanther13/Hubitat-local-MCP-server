...
def main_commit_description(commits: list[dict]) -> str:
    ...
    if not body:
        return ""
    # Strip trailers / autogenerated lines that add noise in a changelog.
    body = re.sub(r"\n*Co-[Aa]uthored-[Bb]y:[^\n]*", "", body)
...
def prepend_changelog_entry(new_version: str, date: str, bullets: list[str]) -> None:
    ...
    # Separate bullets with a blank line so multi-line bullets (title +
    # indented continuation) don't visually run into each other.
...
def paragraph_from_bullets(bullets: list[str]) -> str:
    ...
    all PRs with ' / '. Converts '[#NN](url)' → '#NN (url)' so HPM, which may
    render releaseNotes as plain text, still shows a usable link.
    """
    parts = []
    for b in bullets:
        if not b:
        ...
        # Dedent the continuation (strip the 2-space indent).
        continuation_lines = []
        for ln in lines[1:]:
            s = ln.strip()
            if s:
                continuation_lines.append(s)
        body_flat = " ".join(continuation_lines).strip()
