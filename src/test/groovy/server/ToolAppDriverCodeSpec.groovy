...
class ToolAppDriverCodeSpec extends ToolSpecBase {
    ...
    //
    // /app/updateOAuth is NOT served by the hub's :8080 internal router on current firmware
    // (verified live on 2.5.0.159: it 404s there while the identical query succeeds on the
    // port-80 admin server). The tool tries :8080 first -- older firmware may serve it --
    // and falls back to :80 on a 404 ONLY.
    //
    // The two bases are two distinct METHOD names, which is what makes the fallback
    // testable: the path-keyed hubGet mock is base-blind, so the :8080 leg rides
    // hubInternalGet (mock-registered as usual) and the :80 leg rides hubAdminGet
    // (metaClass-stubbed per test). No harness change was needed.

    /** Exception shaped like the platform client's HTTP error: duck-typed .response.status. */
    private static class FakeHttpStatusException extends RuntimeException {
        final def response
        FakeHttpStatusException(int status) {
            super("HTTP ${status}")
            this.response = [status: status]
        }
    }

    def "hub_update_app oauth falls back to the port-80 admin server when the internal router 404s"() {
        given:
        enableWrite()
        hubGet.register('/app/list/single/data/42') { params -> '[{"id":42,"oauthClientId":null,"oauthClientSecret":null}]' }
        and: 'the :8080 internal router does not serve /app/updateOAuth on this firmware'
        def internalHits = []
        hubGet.register('/app/updateOAuth?id=42&oauthEnabled=true&clientId=&clientSecret=&refreshSecret=false') { params ->
            internalHits << 'internal'
            throw new FakeHttpStatusException(404)
        }
        and: 'the port-80 admin server does, and generates the creds'
        def adminPaths = []
        script.metaClass.hubAdminGet = { String p, Map q = null, Integer t = 30 ->
            adminPaths << p
        ...
        then: 'the fallback served it -- same creds a single-base success would return'
        result.success == true
        result.oauth.success == true
        result.oauth.clientId == 'gen-id'
        result.oauth.clientSecret == 'gen-secret'

        and: ':8080 was tried FIRST (older firmware must keep working), then :80 with the identical query'
        internalHits == ['internal']
        adminPaths == ['/app/updateOAuth?id=42&oauthEnabled=true&clientId=&clientSecret=&refreshSecret=false']
    }

    def "hub_update_app oauth does NOT reach for port 80 when the internal router answers"() {
        // The fallback must stay dormant on firmware that serves the endpoint on :8080 --
        // otherwise every OAuth enable pays a wasted request.
        given:
        enableWrite()
        hubGet.register('/app/list/single/data/42') { params -> '[{"id":42,"oauthClientId":null,"oauthClientSecret":null}]' }
        hubGet.register('/app/updateOAuth?id=42&oauthEnabled=true&clientId=&clientSecret=&refreshSecret=false') { params ->
        ...
        def adminCalls = 0
        script.metaClass.hubAdminGet = { String p, Map q = null, Integer t = 30 -> adminCalls++; '{"success":false}' }
    ...
    @spock.lang.Unroll
    def "hub_update_app oauth does NOT fall back to port 80 on a #status (only a 404 means wrong router)"() {
        // A 401/403 is Hub Security and a 500 is a real hub-side failure -- both are genuine
        // answers from the right server. Retrying them on another port would double the work
        // and report the second port's error instead of the real one.
        given:
        enableWrite()
        hubGet.register('/app/list/single/data/42') { params -> '[{"id":42,"oauthClientId":null,"oauthClientSecret":null}]' }
        hubGet.register('/app/updateOAuth?id=42&oauthEnabled=true&clientId=&clientSecret=&refreshSecret=false') { params ->
            throw new FakeHttpStatusException(status)
        }
        def adminCalls = 0
        script.metaClass.hubAdminGet = { String p, Map q = null, Integer t = 30 ->
            adminCalls++
            '{"success":true,"clientId":"should-never-be-used","clientSecret":"x"}'
        }

        when:
        def result = script.toolUpdateAppCode([appId: '42', oauth: [enabled: true], confirm: true])

        then: 'the original error is reported and port 80 was never touched'
        result.success == false
        result.oauth.success == false
        result.oauth.error.contains(status.toString())
        adminCalls == 0
    ...
    def "hub_update_app oauth reports failure when BOTH the internal router and port 80 fail"() {
        given:
        enableWrite()
        hubGet.register('/app/list/single/data/42') { params -> '[{"id":42,"oauthClientId":null,"oauthClientSecret":null}]' }
        hubGet.register('/app/updateOAuth?id=42&oauthEnabled=true&clientId=&clientSecret=&refreshSecret=false') { params ->
        ...
        then: 'the existing failure envelope, not a throw and not a false green'
        result.success == false
        result.oauth.success == false
