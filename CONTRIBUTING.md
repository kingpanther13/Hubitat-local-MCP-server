...
- **Bundle zip — hands off**: the libraries under `libraries/` ship to hubs as one HPM bundle (`mcp-libraries.zip`), delivered from the bot-only `bundle-artifacts` branch — `publish-bundle-artifact.yml` builds and publishes it on every push (`branches/<branch>/` + `shas/<sha>/`). Nothing under `bundles/` is committed (the build output dir is gitignored). Run `python tools/build-bundle.py` locally only to check the builder accepts your `LIBS` entry (CI runs the same build smoke). The zip carries each library with its whole-line `//` comments blanked and a notice line pointing back here — write comments freely, they just don't ride to the hub. A `//` line inside a `"""`-quoted tool description is kept, as is a comment that merely contains `"""`; the builder fails the build if a transform ever changes that content.
...
If your PR changes tool behaviour, run the relevant existing BAT scenarios and record the tested revision and results. Add or update scenarios only when existing coverage does not exercise the changed behaviour. If execution is pending, say so explicitly; adding scenario documentation or unit tests does not establish live-hub acceptance.
