...
8. **Flatten the bullets (title + body) into a prose paragraph** and prepend to `packageManifest.json` `releaseNotes`. The PR reference suffix (`([#NN](url), @author)`) is **stripped** from each title — HPM shows this string to end users considering an update, and PR numbers/links/author handles aren't useful content in that context. Each PR appears as `Title: body…` with PRs separated by ` / `. Any `[#NN](url)` markdown links remaining inside a commit body (e.g. cross-references to other PRs) are converted to plain-text `#NN (url)` so HPM's plain-text rendering stays readable. CHANGELOG.md keeps the full PR reference and markdown for developers browsing the repo.
