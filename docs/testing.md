...
All of the below **look** plausible for `httpPost` / `httpGet` / other methods on `BaseExecutor`, but all **silently no-op**. If your stub isn't firing, first verify whether the method is on `BaseExecutor` (decompile the hubitat_ci jar: `jar xf hubitat_ci-*.jar; javap -p me/biocomp/hubitat_ci/api/common_api/BaseExecutor.class | grep methodName`) before trying more variants.
