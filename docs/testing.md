...
`tests/sdk_conformance_test.py` connects the official SDK's `streamable_http_client` + `ClientSession` to a real hub `/mcp` endpoint and runs six scenarios: `initialize` (the SDK *raises* if the echoed version isn't one it supports), `ping`, `tools/list` (every entry parsed into `types.Tool`, plus catalog sanity — `hub_`-prefixed, no duplicates, described, object `inputSchema`), a benign read-only `hub_get_info` `tools/call`, the published-`outputSchema` round trip, and the legacy-header contract.
...
The legacy-header scenario is the load-bearing one. Once `initialize` negotiates, the SDK stamps `mcp-protocol-version: <negotiated>` onto every subsequent POST while sending **no** `Mcp-Method`/`Mcp-Name` (those exist only in 2026-07-28). A server reading header *presence* as "modern era" would answer every one with 400 + `-32020` — i.e. break every deployed client. The script owns the `httpx.AsyncClient` it hands the transport and attaches **both** an `event_hooks["request"]` and an `event_hooks["response"]` observer to it, so it asserts on the real outbound headers *and* on the status each of those POSTs came back with (200/202). Reading only the request side would let the scenario print "all served" and pass under exactly the era-detection regression that 400s every other scenario.
...
**Bumping the SDK pin.** The version lives in `tests/sdk-conformance-requirements.txt` and nowhere else; `hub-e2e.yml` installs that file and the script refuses to run against any other version, so CI and a local run cannot silently diverge. That file pins the **whole resolved closure**, not just `mcp`, because the verdicts also turn on `pydantic` (`model_extra` semantics), `typing_extensions` (the `__deprecated__` markers the preflight reads), and `jsonschema` (the SDK's `structuredContent` validator) — leaving those to float would reintroduce exactly the drift the pin exists to prevent. To bump: edit `mcp==`, regenerate the closure into a clean venv (`pip install mcp==<version>` then `pip freeze`, keeping the environment markers), reinstall, run the script, and **read the SDK's release notes for protocol-version changes** — a release that adds `2026-07-28` to its `SUPPORTED_PROTOCOL_VERSIONS` changes which *era* this leg exercises, which is a real change in what it proves. Two scenarios fail loudly rather than going quiet if that happens: the legacy-header one if the SDK starts sending `Mcp-Method`, and the `ping` one if `resultType` becomes a declared field on the SDK's `Result` model (which would blind its `model_extra` check).
