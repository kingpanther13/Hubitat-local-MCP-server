...
### setVariable
Also accepted as `capability='variable'`. Sets a hub variable from one of four mutually-exclusive source modes: a constant, a copy of another variable, a device attribute, or structured variable math. Exactly ONE of `value` / `sourceVariable` / `fromDevice` / `math` per action. The `fromDevice` and `math` source modes require a Number or Decimal target variable -- RM renders the source-mode picker only for numeric targets, so requesting either into a String/Boolean/DateTime variable is rejected with `success=false` before the write (use `value` or `sourceVariable` for those types).
