...
### setVariable
Also accepted as `capability='variable'`. Sets a hub variable from one of four mutually-exclusive source modes: a constant, a copy of another variable, a device attribute, or structured variable math. Exactly ONE of `value` / `sourceVariable` / `fromDevice` / `math` per action.
...
| `value` | Number | Numeric constant to assign -- provide exactly one source mode. String, boolean, and datetime hub-variable targets are not supported via `value`; use `sourceVariable`, or set those types via `rawSettings`. Wire: `numOp=number` + `valNumber.<N>`. |
