...
| 7. Label retries mask gh failures | **Fix.** Explicit Bash enables pipefail for both live-label read steps. Extracted-loop tests verify failure retries and eventual success without network. Automatic E2E uses main's workflow, so the changed loop is covered by the branch's Python CI regression. |
...
| 16. Successful redirect logged as exception | **Fix.** `_hubRequest` no longer marks the telemetry outcome failed on a redirect it handles as the caller's success case. HTTP failure paths remain errors. |
...
| [Retained error reports](https://github.com/kingpanther13/Hubitat-local-MCP-server/issues/407#issuecomment-5745488488) | **Implement.** Keep at most ten bounded, scrubbed errors in state, written only on the error path. Report them ahead of the rolling native history; a bug report with no failingTool takes the newest retained tool error inside its log window as a marked title default only, without narrowing the log scope. Privacy suppression and log clearing apply. Add state inventory, unit/dispatch coverage and live E2E error→report check. |
...
Baseline probes ran on MCP 4.4.1, firmware 2.5.1.181, against a full backup, using only a throwaway uninstantiated app-code probe and a throwaway device-free, paused Visual Rule, both deleted afterwards. Existing devices, rules, app code, Mesh settings and radios were not modified.

Those probes tested the installed baseline, not the patched server. Patch verification is the CI matrix (unit, Python, sandbox, Groovy 2.4 and E2E lanes) plus the live checks recorded in the PR.
