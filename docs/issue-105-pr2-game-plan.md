...
Lead with the high-severity bugs; the tied medium/low items share the same code paths and review context. **Note:** during implementation (TDD), findings **#1 and #2 were empirically refuted** (see table) — `JsonOutput` always escapes non-ASCII, so the "multibyte undercount" cannot occur. **Four actionable high-severity bugs remain: #3, #4, #5, #6.**

#### High-severity bugs

| # | Finding | Location | What's wrong / fix |
|---|---|---|---|
| 1 | ~~Outer size-guard under-measures multibyte UTF-8~~ **— REFUTED (false positive, 2026-06-03)** | `:607-610` (`handleMcpRequest`) | The audit (and its adversarial pass) assumed `JsonOutput.toJson` emits raw multibyte UTF-8. It does **not**: Groovy escapes every char `> 126` to an ASCII `\uXXXX` sequence on both Hubitat's 2.4 runtime and the 3.0 harness (the `disableUnicodeEscaping` opt-out is Groovy 4.0.19+ only). So `jsonResponse` is always pure ASCII → `length()` always equals UTF-8 byte length → the guard cannot undercount. Proven empirically via a dispatch-spec characterization test (which also trips if a future Groovy upgrade stops escaping). **No code change.** |
| 2 | ~~`tools/list` catalog has no byte-accurate backstop~~ **— REFUTED (depended on #1)** | `:686-706` (`handleToolsList`) | Subsumed by #1: the guard is byte-accurate for the always-ASCII wire form, so the catalog's sole backstop is sound. **No code change.** |
