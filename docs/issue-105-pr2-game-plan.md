...
- **#209 (monolith → library split) is its own issue. PR2 does NOT perform the split.** PR2c does the safe *in-file* cleanups only (dead-code removal, helper extraction, memoization, comment/constant hygiene). The file-split itself is deferred to #209.
- **Maintainer constraint (2026-06-03): #209 must not require any HPM-packaging changes.** This rules out #209's **Path A** (`#include` libraries), which requires migrating the app to HPM `bundles[]` packaging — per @level99's VeSync experience, shipping with `libraries[]` silently broke every user's install. The only HPM-neutral route is **Path B (build-time concat):** split the source tree into modules and concatenate them into the single `hubitat-mcp-server.groovy` deliverable at release time, so HPM still sees one importable file with no manifest change. If #209 proceeds it is Path B; otherwise it is shelved. (This also moots the Path-A `#include` wizard-closure viability spike.)
- **PR2 itself touches no HPM / packaging files.** The only HPM-adjacent item is the `_hpmFetchManifests` error-contract fix (Q3), which lives inside an MCP tool that *queries* HPM — not the app's own distribution. **PR2b's HTTP-client consolidation deliberately cuts a clean `HubHttpClient` seam that #209 (Path B) can later extract** — PR2 prepares the split rather than competing with it.
...
- **R2 — File split:** deferred to #209; PR2 does in-file cleanups only. Per maintainer constraint, #209 must require **no HPM-packaging changes** — so it is Path B (build-time concat → single artifact) or shelved; Path A (`#include` + `bundles[]`) is ruled out.
