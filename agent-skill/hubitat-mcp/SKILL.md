...
**More than one device? Pass `commands` to the same tool.** `hub_call_device_command` accepts `commands: [{deviceId, command, parameters?}]` (max 20) and sends the whole group in ONE round trip. This matters more than it looks: the round trip, not the hub actuating the device, is what costs the time — six Z-Wave switches measured ~4.4s sent separately against ~1.5s batched, and a batch of 1, 2 or 4 devices is flat at ~0.95s. `commands` is mutually exclusive with `deviceId`/`command`, takes no `waitFor`, and returns no `state` snapshot on its entries; confirm the whole group with `hub_get_device_attribute`'s `deviceIds` form. Entries are independent, so mixed devices and mixed commands belong in one batch, and one bad entry is reported in its own `results[]` slot without stopping the others (the aggregate then names them in `failedDeviceIds`). A big batch that runs up against the relay's time budget stops early and hands back `stoppedEarly: true` plus `remainingCommands` — send those as the `commands` of a follow-up call. For a set you command repeatedly, a group or scene is better still — this is for the ad-hoc set nobody defined in advance.

The `state` snapshot returned by the single-device form of `hub_call_device_command` is read AS OF the command, so it shows the PRE-effect value. To confirm the resulting state in the same call, pass `waitFor` (block-polls the attribute until it converges or times out); to confirm separately — or to await a condition across several devices at once via `deviceIds` — use `hub_get_device_attribute` with an expected value. Both support comparators (`eq`/`ne`/`gt`/`gte`/`lt`/`lte`/`between`) and a `stableForMs` debounce; non-convergence comes back with a diagnostic flag telling you why. The full semantics live in the two tools' own descriptions. Polling BLOCKS the MCP request for up to `timeoutMs`; use sparingly, prefer event-driven flows, and avoid running it in parallel with other MCP calls.
