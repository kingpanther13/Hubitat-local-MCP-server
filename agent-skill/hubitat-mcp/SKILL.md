...
**More than one device? Pass `commands` to the same tool.** `hub_call_device_command` accepts `commands: [{deviceId, command, parameters?}]` (max 20) and sends the whole group in ONE round trip. This matters more than it looks: the round trip, not the hub actuating the device, is what costs the time — six separate calls measured ~4.5–5s against ~2.75s batched. `commands` is mutually exclusive with `deviceId`/`command` and does not take `waitFor`; confirm with `hub_get_device_attribute`'s `deviceIds` form. Entries are independent, so mixed devices and mixed commands belong in one batch, and one bad entry is reported in its own `results[]` slot without stopping the others. For a set you command repeatedly, a group or scene is better still — this is for the ad-hoc set nobody defined in advance.
