...
2. Find backup files (named `mcp-backup-app-<id>.groovy`, `mcp-backup-driver-<id>.groovy`, or `mcp-backup-library-<id>.groovy`; a replacement taken while the previous one is still indexed carries a `-<uuid>` suffix before `.groovy`, use the exact filename indexed by `hub_list_backups`, since a newer unindexed file may be a leftover from failed cleanup)
3. Download the file
