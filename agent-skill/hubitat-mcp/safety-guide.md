...
2. Find backup files (named `mcp-backup-app-<id>.groovy`, `mcp-backup-driver-<id>.groovy`, or `mcp-backup-library-<id>.groovy`; a replacement taken while the previous one is still indexed carries a `-<uuid>` suffix before `.groovy`, and the newest file is the live backup)
3. Download the file
