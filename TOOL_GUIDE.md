...
- **Inline conditional trigger** (`trigger.condition` Map): `{capability, deviceIds?, state?, comparator?, value?, attribute?, variable?, compareToVariable?, not?, rawSettings?}`. Drives the conditional-trigger sub-wizard inline (sets `isCondTrig.<N>=true` + `condTrig.<N>=a` + the condition fields + clicks hasAll). The same per-capability extended shapes that work in `addRequiredExpression.conditions[]` and `addAction.expression.conditions[]` apply here. Convenience: pass singular `deviceId: N` (integer) instead of `deviceIds: [N]` (array) -- the dispatcher normalizes before writing. If both `deviceId` and `deviceIds` are supplied, `deviceIds` wins.
  - `compareToVariable` is **addTrigger-only** (Variable LHS compared to another hub variable). `addRequiredExpression` and `addAction` IF-expressions support `value` (literal numeric RHS) for Variable conditions but NOT `compareToVariable` (variable-vs-variable).
