...
- **`hub_get_visual_rule(appId?)`** — list every Visual Rules Builder rule (omit `appId`: `{appId, name, disabled, paused}` entries — `paused` is name-suffix detected, so read a single rule for the authoritative `rulePaused`) or read one rule's full definition. Every single-rule success response carries `format`: `'classic'` (`{whenNodes, thenNodes, elseNodes}`) or `'graph'` (`{version, nodes, edges}`). Read master.
