...
As of v0.8.0, the server uses **domain-named gateways** to organize lesser-used tools behind gateway tools. The MCP `tools/list` shows 30 items (11 core + 19 gateways) covering 98 total tools. Use `hub_search_tools` to find any tool by natural language query.
...
7. **`hub_get_rule_health(appId)`** — read-only health check on any installed app. Surfaces broken markers (with a per-marker occurrence count in `brokenMarkerCounts`), multiple-flag poison, configPage errors. Use after a destructive operation to confirm the app is still well-formed.
...
- **Deleted-condition residue.** The replaced condition's underlying settings (e.g. `rCapab_<N>` / `modes<N>`) linger in the settings pool but are NOT part of the active formula -- this is harmless and renders cleanly; no extra cleanup write is issued. The new condition slot indices may continue past the deleted slot (allocation does not reuse the deleted index).
