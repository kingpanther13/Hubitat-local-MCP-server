...
- Summary response (always returned) includes: id, name (driver type), label, room, currentStates, `disabled`, `deviceNetworkId`, `lastActivity`, `parentDeviceId` (v0.10.0+) — enough for most filtering without `get_device` round-trips. To count children of a parent device, group the response on `parentDeviceId` client-side
