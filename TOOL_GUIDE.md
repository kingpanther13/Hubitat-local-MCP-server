...
| `hub_get_jobs` | 100 | Pages `scheduledJobs`; `runningJobs` and `hubActions` stay in full. Served from a 30 s cached `/logs/json` snapshot shared with `hub_get_performance_stats`; over the cloud relay the fetch runs in the background and the call continues via `requestState` (legacy clients repeat the identical call on `status: "in_progress"`). |
