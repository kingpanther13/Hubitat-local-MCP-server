...
- **`get_hpm_drift`** — cross-reference HPM-tracked state against what is actually installed
  ...
  - Data-quality issues land in `dataQualityWarnings[]` on the per-package entry and in the top-level `dataQualityWarnings[]` -- they do NOT inflate `totalDriftSignals`. Currently the only data-quality warning types are: `skipped-malformed-heid` (non-scalar or whitespace-padded heID; entry has `componentName`, `componentId`, `_warning`), `empty-heid` (blank heID normalized to null; entry has `componentName`, `componentId`, `_warning`), `skipped-malformed-component` (non-Map component entry; entry has only `type`, `componentType`, `_warning` -- no `componentName`/`componentId` because the source was not a Map)
