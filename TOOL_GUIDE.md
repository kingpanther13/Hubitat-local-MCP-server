...
- `settingsSkipped[]` sentinel reasons callers may see:
  ...
  - `not_in_schema` -- a written field was absent from the current page schema. **Context-dependent**: on `addTrigger` it is **informational** and does NOT flip `partial` (a field the trigger shape legitimately does not need, e.g. `ReltDev_<N>`/`tCustomAttr_<N>` for a `*changed*` Custom-Attribute trigger that has no comparator RHS) **because** a baked trigger that simply did not need a field is not a degraded write; on the walker pages (STPage/doActPage) the same reason IS genuine degradation and DOES flip `partial`, because there a field falling out of the expected reveal chain means the condition did not assemble as requested.
