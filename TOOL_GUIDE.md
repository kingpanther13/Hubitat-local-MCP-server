...
**Protected apps.** Select installed apps under **Protected apps** in the MCP app UI to block generic app and native-rule mutations targeting them. Protection applies in flat and gateway mode, with Developer Mode either on or off; reads remain available. The MCP instance is preselected for new installs and upgrades. Subsequent updates preserve your selections, including an empty list. Change protection in the Hubitat UI. Dedicated Developer Mode settings and package maintenance retain their existing gates, but `hub_update_mcp_settings` cannot change the list.
