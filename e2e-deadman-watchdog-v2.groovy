...
def adminManageVariables(args) {
    ...
    if (!action) {
        return [tools: [
            [name: "hub_get_variable", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Read a hub variable by name."],
            [name: "hub_set_variable", annotations: [readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Set a hub variable's value (write)."]
        ]]
    }
    def name = args?.name
    switch (action) {
...
def getAdminToolDefinitions() {
    return [
        [name: "hub_update_app", annotations: [readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: true], inputSchema: [type: "object", properties: [:]], description: "Update an Apps Code class source (deploy). One of source/sourceFile/importUrl/resave; confirm:true required.",
        ...
        [name: "hub_get_source", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Read app/driver/library source (chunked); auto-saves the full source to File Manager so a restore can read it.",
         inputSchema: [type: "object", properties: [
            type: [type: "string", enum: ["app", "driver", "library"]], id: [type: "string"],
            offset: [type: "integer"], length: [type: "integer"]], required: ["type", "id"]]],
        [name: "hub_create_library", annotations: [readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true], inputSchema: [type: "object", properties: [:]], description: "Install a new library. One of source/sourceFile/importUrl; confirm:true required.",
         inputSchema: [type: "object", properties: [
            source: [type: "string"], sourceFile: [type: "string"], importUrl: [type: "string"], confirm: [type: "boolean"]], required: ["confirm"]]],
        [name: "hub_update_library", annotations: [readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: true], inputSchema: [type: "object", properties: [:]], description: "Update an existing library. One of source/sourceFile/importUrl/resave; confirm:true required.",
         inputSchema: [type: "object", properties: [
        ...
        [name: "hub_delete_item", annotations: [readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Delete an app/driver/library by id. confirm:true required.",
        ...
        [name: "hub_force_delete_app", annotations: [readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Force-delete an INSTALLED-APP instance (e.g. an RM rule) via /installedapp/forcedelete/<id>/quiet. confirm:true required.",
        ...
        [name: "hub_set_app_disabled", annotations: [readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Toggle an installed app's disabled flag (the admin UI red-X) via POST /installedapp/disable; verified by read-back. confirm:true required.",
        ...
        [name: "hub_get_metrics", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Current hub metrics (free memory, temp, DB size, uptime) + the hub's own health alerts. Read-only."],
        ...
        [name: "hub_get_memory_history", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Free-memory / CPU-load history rows from the hub. Args: limit (default 60). Read-only.",
         inputSchema: [type: "object", properties: [limit: [type: "integer"]]]],
        [name: "hub_get_hub_logs", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Most-recent hub system log entries. Args: level (error/warn/info), limit (default 50). Read-only.",
         inputSchema: [type: "object", properties: [level: [type: "string"], limit: [type: "integer"]]]],
        [name: "hub_list_app_instances", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Every running app INSTANCE (flattened /hub2/appsList with parentId) -- the full app inventory. DISTINCT from hub_list_apps (Apps Code CLASSES, which resolve_class_id depends on). Read-only."],
        [name: "hub_install_bundle", annotations: [readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: true], inputSchema: [type: "object", properties: [:]], description: "Install a code bundle .zip from a URL the hub fetches itself (HPM-style). confirm:true required.",
        ...
        [name: "hub_list_bundles", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "List installed code bundle containers (id/name/namespace). Read-only.",
         inputSchema: [type: "object", properties: [:]]],
        [name: "hub_delete_bundle", annotations: [readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Delete a code bundle container by id (verified by re-list). confirm:true required.",
         inputSchema: [type: "object", properties: [bundleId: [type: "string"], confirm: [type: "boolean"]], required: ["bundleId", "confirm"]]],
        [name: "hub_get_info", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Hub model/firmware/memory + the issue #237 lastSelfDeploy record (with ageMs)."],
        [name: "hub_list_apps", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "List Apps Code types (scope='types') or installed apps.",
         inputSchema: [type: "object", properties: [scope: [type: "string", enum: ["types", "instances"]]]]],
        [name: "hub_list_libraries", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "List libraries (id/name/namespace/version summaries)."],
        [name: "hub_get_jobs", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "List scheduled + running hub jobs."],
        [name: "hub_read_file", annotations: [readOnlyHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Read a File Manager file (chunked).",
         inputSchema: [type: "object", properties: [fileName: [type: "string"], offset: [type: "integer"], length: [type: "integer"]], required: ["fileName"]]],
        [name: "hub_write_file", annotations: [readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Write a File Manager file. confirm:true required.",
        ...
        [name: "hub_create_backup", annotations: [readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Trigger a hub DB backup. confirm:true required.",
         inputSchema: [type: "object", properties: [confirm: [type: "boolean"]], required: ["confirm"]]],
        [name: "hub_manage_variables", annotations: [readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false], inputSchema: [type: "object", properties: [:]], description: "Read/set hub variables for the lease (hub_get_variable / hub_set_variable). Call with no action to list sub-tools.",
