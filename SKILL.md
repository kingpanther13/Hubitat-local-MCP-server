...
| `/hub/advanced/blinkLED` | Fires the hub's identify-LED sequence (blue → red → green) once. Returns the literal text `true`. Single GET, no body, self-resetting. Surfaced via opt-in `blinkLED`/`identifyHub` flags on `get_hub_info` and `device_health_check`. Firmware 2.5.0.123+. |
