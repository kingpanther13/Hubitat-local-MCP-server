...
`update_native_app` `clearActions` / `replaceActions` shortcuts: retry-window exhaustion returns `partial:true, asyncCommitLikely:true` with a `stage` discriminator and a `safeRecovery` block. Verify via `get_app_config` rather than retrying or rolling back -- the deletion typically commits 5-15s after the response. See TOOL_GUIDE.md for the full response shape.
