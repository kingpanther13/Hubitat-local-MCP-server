...
- **Transport**: Streamable HTTP (not SSE/stdio) with OAuth access token — `?access_token=<token>` on the query string, or equivalently an `Authorization: Bearer <token>` header (PLATFORM behaviour, not this app's code: the Hubitat OAuth layer accepts either, on both the local endpoint and the cloud relay — verified live on both against firmware 2.5.1.135, pinned by the e2e bearer scenario)
