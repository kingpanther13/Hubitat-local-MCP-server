...
**Device allowlist bypass (`settings.bypassDeviceAllowlist`, default OFF).** When the operator turns this bool ON (Device Access settings page, or `hub_update_mcp_settings`), `findDevice` returning null for an unlisted device is no longer fatal: the **per-device** tools fall back to the hub's id-keyed admin endpoints and reach ANY device on the hub by id. The covered set is FIVE tools (`hub_get_device`, `hub_get_device_attribute`, `hub_call_device_command`, `hub_update_device`, `hub_list_device_events`); the fallback is wired into the implementations `toolGetDevice`, `toolGetAttribute`, `toolPollUntilAttribute` (a per-device value reader re-fetches `/device/fullJson` each poll), `toolSendCommand` (commands fire via `POST /device/runmethod`, with `waitFor` reusing the same poller), `toolGetDeviceEvents` and `toolGetDeviceHistory`'s device branch (both back `hub_list_device_events`; read from `/device/eventsJson`, client-side windowed), and `toolUpdateDevice` (`label`→`/device/updateLabel`, `name`/`deviceNetworkId`→`/device/update`, `preferences`→`/device/preference/save`, `room`→`/device/updateRoom` by room NAME with an existence check (unassign routes via `/device/update` `roomId=0`), `enabled`→`/device/disable`). `hub_list_devices`, device swap/replace/delete, and device-health are NOT bypassed (they still honour the allowlist). LISTED and MCP-managed devices ALWAYS keep the rich Groovy-device path unchanged — only the unlisted fallback is new. The toggle's EFFECT is independent of Developer Mode (the `hub_update_mcp_settings` tool that can set it is Developer-Mode-gated, but once the flag is ON the bypass works in normal operation). This deliberately removes the device-selection security boundary — it carries a loud warning everywhere it appears. On the bypass path `hub_call_device_command` can return `success: false` (a structured hub-rejection / not-confirmed runtime error) where the listed path is fire-and-forget. `showOnHome`/`defaultCurrentState`/`tags`/`dataValues` writes are not supported on unlisted devices **because** their only id-keyed route is the wholesale `/device/update` form whose field encoding for those properties is unverified; bypass attribute discovery (for `hub_get_device_attribute`) is limited to attributes that have already reported a value.
...
| `/device/fullJson/<id>` | Comprehensive device JSON — includes `appsUsing` array (apps referencing this device: `{id, name, label, trueLabel, disabled}`), `appsUsingCount`, `parentApp`, plus device commands/attributes/settings/dashboards. `device.currentStates` is an OBJECT keyed by attribute name (`{value, dataType, unit, numberValue, date}`); top-level `commands` is an array (`{capability, name, arguments, parameters, relatedAttribute}`). Used by `hub_list_device_dependents` AND the device-allowlist bypass (resolves an unlisted device for `hub_get_device` / `hub_get_device_attribute` / `hub_call_device_command` / `hub_update_device` / `hub_list_device_events`). |
| `/device/eventsJson/<id>` | A device's event history as a JSON array (newest-first, no query params): `{id, name, value, unit, descriptionText, source, type, date, isStateChange, deviceId, ...}`. Used by the device-allowlist bypass branch of `hub_list_device_events` (both the recent-N mode and the windowed mode -- limit + attribute + strictly-after window applied client-side). |
| `/device/updateLabel?deviceId=<id>&label=<label>` | Sets a device's label; returns the literal text `true`. Tried first by `hub_create_device` for the optional post-create label. Dedicated setter that is 404-prone on some firmwares (confirmed absent on 2.5.0.157) -- so on failure BOTH the device-allowlist bypass branch of `hub_update_device` AND the `hub_create_device` label step fall back to the portable wholesale `/device/update` form (which carries a `label` field); `hub_create_device` warns only if both paths fail. |
...
| `/hub2/vrb/devices` | The successor to the row above and the second tier `hub_list_devices scope='all'` tries: the VRB 2.0 device picker feed, a FLAT JSON array of `{id, label, capabilities, temperature, lightEffects, supportedFanSpeeds, buttonCount}` (the last four null unless the device has them) covering every hub device, child devices included, regardless of MCP authorization. It is joined by id onto the `/hub2/devicesList` spine below: when every spine device has a feed entry with a capabilities list the inventory reports `source: /hub2/vrb/devices` and complete capabilities; a device the feed omits stays in the inventory without capabilities and the result is flagged `capabilitiesPartial` with a `capabilitiesNote` saying how many were omitted (authorized ones are filled from the Groovy model). If the spine cannot be read, the feed alone is returned, flagged partial. Also validates `selectedDevices` ids for `hub_update_mcp_settings`. |
| `/hub2/devicesList` | Every device on the hub as a nested tree `{devices: [{key, data: {id, name, secondaryName}, children: [...]}]}` -- no capabilities. On platform 2.5.1.173+ this is the authoritative SPINE of every `scope='all'` inventory (ids and order), with `/hub2/vrb/devices` joined on for capabilities; it stands alone (`source: /hub2/devicesList`, `capabilitiesPartial`, capabilities filled for MCP-authorized devices only) when the capability feed does not answer. `hub_update_mcp_settings` validates `selectedDevices` ids against the same inventory. A well-formed inventory with no devices means a hub with no devices; the unreadable cases (no body, no `devices` key) fail as a shape error. |
...
| `/device/runmethod` | JSON: `{"id":<int>, "method":"<command>", "args":[{"type":"<argType>","value":<v>}, ...]}` | Invoke a command on ANY device by id (allowlist-blind). Returns `{"success":true, "message":null}`. Empty `args` for a no-parameter command. Used by `hub_call_device_command` on the allowlist-bypass path. |
| `/device/preference/save` | JSON: a partial preferences object keyed by `deviceId` (e.g. `{"deviceId":<int>, "<pref>":<value>}`) | Saves one or more device preference/settings fields without the wholesale device-edit form (touches only the named fields). Used by `hub_update_device` (showOnHome/defaultCurrentState fallback + the allowlist-bypass `preferences` path). |
| `/device/update` | form-urlencoded: the FULL device-edit model (name, label, deviceNetworkId, roomId, tags, ...; boolean true→`on`) | Wholesale device-edit "Save" — BLANKS any field it omits, so the full model must be re-sent. Used by `hub_update_device` (the `tags` path and the allowlist-bypass `label`/`name`/`deviceNetworkId` path and the room-unassign `roomId=0` path; the bypass rebuilds the model from a FRESH fullJson fetch so a prior same-call leg isn't reverted -- e.g. a room-unassign form re-sends the `label` an earlier leg just applied). `label` rides this form on the bypass path because the dedicated `/device/updateLabel` setter 404s on some firmwares. |
...
- **Error codes**: `-32700` (parse), `-32600` (invalid request — also the modern batch-body rejection), `-32601` (method not found), `-32602` (invalid params from `IllegalArgumentException`), `-32603` (internal error), plus `-32020` (HeaderMismatch, modern only) and `-32022` (UnsupportedProtocolVersion, both eras)
