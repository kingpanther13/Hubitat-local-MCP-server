...
| `/hub2/devicesList` | Every device on the hub as a nested tree `{devices: [{key, data: {id, name, secondaryName}, children: [...]}]}` -- no capabilities. The only all-hub inventory on platform 2.5.1.173 and later: `hub_list_devices scope='all'` falls back to it (capabilities filled in for MCP-authorized devices only, reported via `source` + `capabilitiesPartial`) and `hub_update_mcp_settings` validates `selectedDevices` ids against it. An empty inventory is treated as a failed read. |
