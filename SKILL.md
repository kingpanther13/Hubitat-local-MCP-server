...
- **Transport**: Streamable HTTP (not SSE/stdio) with OAuth access token — `?access_token=<token>` on the query string, or equivalently an `Authorization: Bearer <token>` header (the Hubitat platform accepts either, on both the local endpoint and the cloud relay; verified live on both)
...
- **Resources (issue #366)**: a stateless read-only surface — capabilities advertise `resources: {subscribe: false, listChanged: false}` (no SSE, so subscriptions are impossible by design). The catalog is the tool-guide sections (`hubitat://guide/<section>`, text/markdown, same content as `hub_get_tool_guide`) plus the live context snapshot (`hubitat://context-summary` text/plain and `hubitat://context` application/json). The context resources are gated by the Read master (hidden from `resources/list`, `-32002` on read when reads are disabled); the guide resources are never gated, mirroring `hub_get_tool_guide`. Unknown URI on `resources/read` → `-32002` with `data.uri`; `resources/templates/list` returns an empty list. All three results carry `ttlMs`/`cacheScope` (0 ttl on the live context reads, `cacheHintTtlMs()` elsewhere)
