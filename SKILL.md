...
| `/device/update` | form-urlencoded: the FULL device-edit model (name, label, deviceNetworkId, roomId, tags, ...; boolean true→`on`) | Wholesale device-edit "Save" — omitted fields can be reset, so rebuild the full model from fresh native details. Omit null `groupId`, `controllerType`, and unchanged null `roomId` to preserve them exactly; carry non-null values unchanged and retain an explicit `roomId=0` room clear. Used by `hub_update_device` (the `tags` path and combined `label`/`name`/`deviceNetworkId` edits and the room-unassign `roomId=0` path; the tool rebuilds the model from a FRESH fullJson fetch so a prior same-call leg isn't reverted -- e.g. a room-unassign form re-sends the `label` an earlier leg just applied). `label` falls back to this form because the dedicated `/device/updateLabel` setter 404s on some firmwares. |
