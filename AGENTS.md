...
- **Next-revision protocol — PARTIALLY ADOPTED.** The 2026-07-28 revision (RC locked 2026-05-21) makes MCP stateless: the protocol version rides every request's `_meta`, every result carries `resultType`, list responses carry `ttlMs`/`cacheScope` cache hints, and `server/discover` becomes mandatory. **Adopted:** `resultType: "complete"` plus the `io.modelcontextprotocol/serverInfo` `_meta` key on every result (stamped centrally in `jsonRpcResult`, so a new handler gets both for free), the `ttlMs`/`cacheScope` hints on `tools/list`, and `server/discover`. **Not yet:** `2026-07-28` is deliberately withheld from `supportedProtocolVersions()` until request-header validation (`Mcp-Method` / `Mcp-Name` / `MCP-Protocol-Version`, `-32020` on mismatch) lands — the hub's mapped-endpoint request object does not expose inbound headers reliably in the sandbox, so that is the prerequisite. The modern error rejections and their HTTP status mappings (`-32020`/`-32021`/`-32022` → 400, unknown method → 404 + `-32601`) ship together with that flip and MUST NOT ship earlier: emitting a recognized modern error from a legacy-era server makes dual-era clients skip the `initialize` fallback and cache the wrong era, so an unknown per-request `_meta` protocol version is deliberately tolerated today. Don't re-derive any of this from the spec; extend it.
...
- MCP next-revision Release Candidate (RC locked 2026-05-21, targets 2026-07-28 publication) — blog.modelcontextprotocol.io/posts/2026-07-28-release-candidate/; the authoritative shapes are in the draft schema at github.com/modelcontextprotocol/modelcontextprotocol `schema/draft/schema.json`. Partially adopted — see *Schema design → Next-revision protocol* for what is and isn't in.
