...
- **`outputSchema` — declare one on every new tool definition for consistency (a project convention, NOT a spec requirement); but its EMISSION on the wire is opt-in and OFF by default (issue #290).** The MCP spec marks `outputSchema` **optional**; its only normative `MUST` is conditional — *if* a server publishes one, its structured results must conform. The spec says clients SHOULD validate, but strict clients (e.g. Claude Desktop, via the MCP TypeScript SDK) go further and *require* `structuredContent` to be present whenever `outputSchema` is advertised, throwing JSON-RPC `-32600` ("has an output schema but did not return structured content") otherwise — and this server returns text-only results, never `structuredContent`. So each tool keeps its `outputSchema` in its DEFINITION as a discovery/documentation aid, but it is **NEVER emitted on any `tools/list` surface or the gateway catalog unless the advanced setting `publishOutputSchemas` is `true`** (default OFF). When ON, the pre-#290 behaviour is restored: gateway-mode base tools and the gateway-catalog disclosure carry `outputSchema` (and strict clients break again — that is the documented trade-off of enabling it). The flat `tools/list` **never** emits `outputSchema` regardless of the toggle (size-constrained surface; always stripped). **Do NOT add `structuredContent`:** results are text-only, so `outputSchema` is documentation, not a wire-format contract to fulfill. Model the *success* shape; for multi-mode tools use a superset object (union of possible keys, none `required`). `publishOutputSchemas` is also settable via the developer-mode self-admin tool `hub_update_mcp_settings`. Source: MCP spec — `/server/tools` (`outputSchema` is Optional); issue #290.
...
A label only RE-RUNS e2e when it **flips the lane** (focused↔full), decided by the `lane-gate` job: the FIRST full-run label (focused→full) and removing the LAST one (full→focused) re-run; a redundant full-run label (a 2nd `release:*`/`e2e:full` while already full, or removing one while another remains) is a **no-op** — the prior full run's gate status still stands, so it does NOT re-deploy. A new commit (or `workflow_dispatch`) always re-runs. Applying the FIRST full-run label while a **focused** run is live **cancels** it and starts the full run; a redundant full-run label while a **full** run is already live **leaves that run alone** (the `cancel-in-progress` expression keys on the same first-full-label test as lane-gate). A commit/reopen/dispatch always cancels-and-supersedes.
