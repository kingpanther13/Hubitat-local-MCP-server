...
- **Validation errors** (caller-recoverable, bad args): throw `IllegalArgumentException`. `handleToolsCall` catches it and returns a tool result with `isError: true` whose `error` text is the message plus the reactive guide pointer. This follows the 2026-07-28 tools page, which classes input validation and business-logic errors as tool execution errors (`isError: true`) that clients SHOULD show the model, and reserves JSON-RPC `-32602` for protocol errors (unknown tool, malformed `tools/call` envelope, `requestState` faults) that clients only MAY show. `_isProtocolValidation` is the single list of messages that stay on `-32602`. **A validation throw MUST fire before any side effect** — callers may correct and retry a validation result, so a tool that mutates state and then throws `IllegalArgumentException` would expose that work to a double-run.
