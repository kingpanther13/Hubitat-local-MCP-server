...
- **`outputSchema` — LEGACY and frozen: do not add, extend, or test one.** The MCP spec marks `outputSchema` *optional*, and this project treats it as deprecated. The declarations that exist stay as they are for anyone who turned emission on; a new tool does NOT declare one, an existing tool's schema is NOT updated when its response gains or changes keys, and no new test asserts on a schema's CONTENT (the existing assertions stay: `McpToolAnnotationsSpec` checks that a declared schema is well-formed, carries no field a retired protocol left behind, and -- for the one tool whose nullable shape was the point of issue #342 -- matches it; `tests/sandbox_lint.py`'s `check_output_schema_freeze` pins the frozen set's per-file counts and a digest of their text. Those guards enforce the freeze, they do not maintain the schemas, and no NEW one is added). A review-bot request to "update the outputSchema" for a changed tool is answered with this policy, not a schema edit. What still runs, unchanged, so the opt-in keeps working: emission is OFF by default (`publishOutputSchemas`, issue #290 — strict clients such as Claude Desktop reject a schema-advertising tool whose result lacks `structuredContent`), and when it is ON the server honours the spec's conditional MUST (issue #342): `handleToolsCall` attaches `structuredContent` on every non-`isError` result of a schema-advertising tool, the emitted schema is the **wire form** (`required` arrays stripped recursively by `_wireOutputSchema`, so the `[success:false, error, note]` error contract validates too), the flat `tools/list` never emits it, and gateway entries never carry one. `publishOutputSchemas` is also settable through `hub_update_mcp_settings`. Source: MCP spec — `/server/tools` (Output Schema); issues #290, #342, #354.
