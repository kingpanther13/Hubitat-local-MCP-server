...
- **Protocol-layer changes also answer to the conformance harness, whose referee is somebody else's code — not our reading of the spec.** Two legs: `McpWireSchemaConformanceSpec` validates rendered wire payloads against the MCP authors' JSON Schemas vendored (network-free) under `src/test/resources/mcp-schema/`, and `tests/sdk_conformance_test.py` drives the official `mcp` Python client against a live hub from its own e2e step. Neither may skip — a missing SDK or schema FAILS. Read [docs/testing.md](docs/testing.md) § *Conformance harness* before touching either: it covers how to run them, how to bump the SDK pin (`tests/sdk-conformance-requirements.txt`), how to refresh the vendored schemas, and why the SDK step's two workflow-file changes need the `gh workflow run` dispatch dance below.
