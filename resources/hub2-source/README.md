...
| `GET  /hub2/restoreLocalBackup?fileName=` · `/hub2/restoreCloudBackup?p=<pwd>&t=<ms>` · `/hub2/restoreUploadedBackup` | Whole-hub DB restore — by local fileName, by cloud encryption password (+cache-buster `t`), or a previously-uploaded backup. **ALL reboot the hub** → `{success}`. Feed `hub_restore_backup(scope=hub_local\|hub_cloud\|hub_uploaded)`. (`GET /hub/restoreWithReboot?localOnly=&onboarding=` is the ONBOARDING-only variant — 404s on a configured hub; not used.) |
