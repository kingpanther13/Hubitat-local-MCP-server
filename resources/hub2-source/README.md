...
| `GET  /hub/zwave2/getNodeState?node=<id>` | Per-node Z-Wave state (plain text; `"Done"` when idle). Read-only. Feeds `hub_get_radio_details(node_id=<id>)` |
| `GET  /hub/zwaveRepair2Status` · `/hub/checkZwaveRepairRunning` · `/hub/zwaveExclude/status` · `/hub/zigbeeInfo/status` · `/hub/matterPairDeviceStatus?nodeId=` | Lifecycle status pollers — repair stage (`{stage,html}`), heal-running flag, exclusion status, Zigbee network status (`{panId,extendedPanId,networkState}`), Matter commissioning status (`{initMap,deviceMap}`). Read-only. Feed `hub_get_radio_details(include_status=true)` |
...
| `GET  /hub/zigbee/enable/<bool>` · `/hub/zigbee/updateChannelAndPower?channel=&powerLevel=` | Zigbee radio enable/disable + channel/power config (idempotent). Feed `hub_set_zigbee` |
| Z-Wave node/mesh lifecycle (non-idempotent) → `hub_call_zwave`: `GET /hub/zwaveRepair2?resetStats=false` · `/hub/zwaveCancelRepair` · `/hub/zwaveNodeRepair2?` (repair); `GET /hub/startZwaveJoin` · `/hub/stopJoin` + `POST /hub/zwave/securityKeys` · `/hub/zwave/securityCode` (inclusion + S2 grant/DSK); `GET /hub/zwaveExclude` · `/hub/stopZWaveExclude` (exclusion ⚠️); `POST /hub/zwave/{refreshNodeStatus,discoverDevice}` (`zwaveNodeId`) · `GET /hub/zwave/nodeReinitialize` · `/hub/zwaveNodeDetailGet` (maintenance); `POST /hub2/zwave/nodeReplace` (`{zwaveNodeId}`) + `GET /hub/zwave/nodeReplace/{info,status,stop}`; `POST /hub/zwave/nodeRemove` (`zwaveNodeId`) ⚠️; `GET /hub/zwave2/{startAntennaTest?node=,antennaTestContinue}` | The full Z-Wave lifecycle surface. Modern `zwaveRepair2` supersedes the legacy `zwaveRepair` that the old `hub_call_zwave_repair` used (now absorbed into `hub_call_zwave`) |
