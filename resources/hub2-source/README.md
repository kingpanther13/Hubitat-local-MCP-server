...
| `GET  /hub/updateName?name=` | Set the hub's name (URL-encode the value). Hub-global write. Backs `hub_set_system_settings` (hubName). Read back via `hub_get_info` (name) |
| `GET  /hub/updateLatLongTimezone?latitude=&longitude=&timeZone=&temperatureScale=&postalCode=` | Wholesale GRANULAR location-settings write — sets latitude, longitude, timeZone, temperatureScale, and postalCode together in one call (URL-encode each; `zipCode` maps to the `postalCode` param). Because it is wholesale, a caller MUST read-merge: read the CURRENT values (`location.latitude/longitude/timeZone?.ID/temperatureScale/zipCode`) and override only the fields being changed, else omitted fields are blanked. ⚠️ **A timeZone change REBOOTS the hub** (1-3 min downtime). Backs `hub_set_system_settings` (latitude/longitude/timeZone/temperatureScale/zipCode); timeZone leg is confirm-gated. Read back via `hub_get_info` |
