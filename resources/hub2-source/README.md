...
| `GET  /modes/json` | Full modes payload: `{modes:[{id,name,icon,conditions}], currentModeId, selectedModeManager (builtIn\|easy\|legacy), modeManagerAppId, easyModeManagerAppId}` (live-probed fw 2.5.0.157). Read-only. Feeds `hub_list_modes` (per-mode icon + Mode Manager state) |
...
| `GET  /modes/setModeManager/{builtIn\|easy\|legacy}` · `GET\|POST /modes/easyModeManager/json` | Mode Manager: select which manager runs; Easy Mode Manager per-mode conditions (GET reads, POST replaces, keyed by mode id). Back `hub_set_mode_manager` + `hub_list_modes.modeManager` |
