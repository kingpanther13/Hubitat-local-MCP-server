...
| `GET  /hub2/hubMeshJson` | (The Hub Mesh admin PAGE itself is `/device/hubMesh` — Settings → Hub Mesh; the mesh endpoints below are what that Vue page calls.) **Hub Mesh** (hub-to-hub device/variable sharing between Hubitat hubs on the same LAN — NOT the Z-Wave/Zigbee radio mesh) — the whole page payload in one read. Keys (from the Vue `populatePage`): `hubList` (auto-discovered peer hubs; rows carry at least `name`, `ipAddress`, `hubId`, plus `warning` when the hub flags one), `hubMeshToken` (THIS hub's own mesh auth token — a credential), `hubMeshEnabled` (bool), `fullRefreshInterval` (seconds: `0`=Never, `120`, `300`, `3600`), `sharedDevices` (this hub's devices shared INTO the mesh: `id`, `name`, `childCount`), `privateDevices` (this hub's UNshared devices — can be hundreds), `localLinkedDevices` (remote devices linked ONTO this hub: `id`, `name`, `childCount`, `appsUsing[]`), `availableLinkedDevices` (remote shared devices not yet linked here: `hubId`, `hubName`, `deviceId`, `deviceDisplayName`), the four hub-variable analogues `localHubVariables` / `sharedHubVariables` / `localLinkedHubVariables` / `availableLinkedHubVariables` (variable rows carry `name`, `type`, and `hubName`/`hubId` for remote ones), `modeHubId` (the peer hub whose modes this hub follows; **absent/null means local modes** — the UI maps it to `"none"`), and `migrationSourceHubs` / `migrationDestinationHubs`. Firmware variations omit keys, so read every field null-safely. Read-only. Backs `hub_get_hub_mesh` (which returns `privateDevices`/`localHubVariables` as COUNTS only). |
| `GET  /hub/advanced/enableHubMesh` · `/hub/advanced/disableHubMesh` | Turn Hub Mesh on/off. ⚠️ **Both require a hub REBOOT to take effect** — the UI's own message is "Hub mesh enabled/disabled, please reboot the hub." Back `hub_update_hub_mesh(enabled)`. |
| `GET  /device/setHubMeshFullRefreshInterval/<seconds>` | Hub Mesh full-sync interval; `<seconds>` rides in the path and is one of `0` (Never), `120`, `300`, `3600` — the only values the hub's own picker offers. Backs `hub_update_hub_mesh(full_refresh_interval)`. |
| `GET  /device/followModes/<hubId>` | Make this hub follow a peer hub's location modes; `<hubId>` is a peer's `hubId` from `hubMeshJson.hubList`, or the literal `none` to revert to this hub's own local modes. Backs `hub_update_hub_mesh(mode_hub_id)`. |
| `POST /device/setHubMeshToken` (JSON) | Store a PEER hub's mesh auth token on this hub — needed when that peer has UI login security. Body `{"hubId": <peer hubId>, "token": "<peer's hubMeshToken>"}`; the Vue page (`saveHubMeshToken()`) sends `hubId` as the **number** it read out of `hubMeshJson`, so an all-digits id is sent as a JSON number, not a string. Backs `hub_update_hub_mesh(peer_hub_id, peer_token)`. |
| `GET  /device/generateHubMeshToken` · `/device/hubMeshReconnect` · `/device/hubMeshFullRefreshNow` | Hub Mesh maintenance verbs: regenerate THIS hub's mesh token (invalidates the token peers already stored), reset + reconnect the mesh links (temporarily drops current connections), and trigger a full sync now. Not surfaced as MCP tools. |
| `GET  /device/addToMesh/<deviceId>` · `/device/removeFromMesh/<deviceId>` · `/device/createLinked/<hubId>/<deviceId>` · `/device/forceDelete/<deviceId>/json` | Per-DEVICE Hub Mesh sharing: share/unshare one of this hub's devices, create a local linked device from a peer's shared device (`hubId`+`deviceId` from `availableLinkedDevices`), and force-delete a linked device. Not surfaced as dedicated MCP tools — per-device mesh flags are set through `hub_update_device` (`meshEnabled`/`meshFullSync` on the wholesale `POST /device/update` form). |
| `GET  /hub2/addVarToMesh/<name>` · `/hub2/removeVarFromMesh/<name>` · `/hub2/createLinkedHubVar/<hubId>/<name>` | Per-VARIABLE Hub Mesh sharing: share/unshare a local hub variable (name is URI-encoded by the UI) and create a local linked variable from a peer's shared one. Not surfaced as MCP tools. |
