...
| `GET  /hub/networkTest/traceroute/<ipv4>` | Hub-side traceroute to one IPv4 host — returns the **plain-text** route table (synchronous, ~0.8s). Host rides in the path (dotted-quad IPv4 only; hostnames rejected). Read-only. Feeds `hub_get_device_health(traceroute=<ipv4>)` |
| `GET  /hub/networkTest/speedtest` | Hub-side WAN download speed test — returns the **plain-text** `wget` log incl. the measured download speed (synchronous, ~10s; pulls a fixed 10 MB blob from `hubitat-public-files.s3.us-east-2.amazonaws.com/speedtest.bin`, no caller input). Read-only. Feeds `hub_get_device_health(speedtest=true)` |
